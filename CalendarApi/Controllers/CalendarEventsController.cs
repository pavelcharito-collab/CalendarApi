...
public class CalendarEventsController(
    CalendarEventSchedulingService scheduling,
    ICurrentUserAccessor current,
    IValidator<ListEventsInRangeQuery> rangeValidator) : ControllerBase
{
    /// <summary>
    ...
    /// <response code="409">Event overlaps another event for a participant.</response>
    [HttpPost("events")]
    ...
    /// <response code="200">Paged event list.</response>
    [HttpGet("events")]
    ...
    {
        var totalCount = await scheduling.CountAllAsync(ct);
        var items = await scheduling.ListAllAsync(take, skip)
            .Select(DtoMapper.ToResponse)
            .ToListAsync(ct);

        return Ok(new EventListResponse(totalCount, items.Count, items));
    }
    ...
    /// <response code="404">Event does not exist.</response>
    /// <response code="409">Update would overlap another event for a participant.</response>
    ...
    /// <param name="range">Required <c>from</c> (inclusive) and <c>to</c> (exclusive) query parameters.</param>
    ...
    /// <response code="400">Missing or invalid range parameters.</response>
    /// <response code="401">Missing or invalid <c>X-User-Id</c> header.</response>
    ...
    public async Task<ActionResult<IReadOnlyList<EventInstanceResponse>>> ListForUser(
        Guid userId,
        [FromQuery] ListEventsInRangeQuery range,
        CancellationToken ct)
    {
        var validation = await rangeValidator.ValidateAsync(range, ct);
        if (!validation.IsValid)
        {
            throw new ValidationException(validation.Errors);
        }

        var items = await scheduling.ListForUserInRangeAsync(current.UserId, userId, range.From, range.To, ct)
            .OrderBy(i => i.Start)
            .Select(DtoMapper.ToResponse)
            .ToListAsync(ct);
